...
@router.post("/schema")
async def ensure_coordinator_schema():
...
async def list_conflicts(resolved: bool | None = None):
...
async def resource_summary(resource_id: str):
...
async def release_lock(resource_id: str, proposal_id: str):
